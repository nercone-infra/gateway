...
FROM debian:trixie-slim AS egress
...
FROM debian:trixie-slim AS onion

ARG TRIXIE_PACKAGES_VERSION

RUN apt-get update && apt-get install -y --no-install-recommends tor ca-certificates \
    && rm -rf /var/lib/apt/lists/*
...
COPY onion/entrypoint.sh /usr/local/bin/entrypoint.sh
...
FROM debian:trixie-slim AS proxy
...
RUN echo "/usr/local/lib" > /etc/ld.so.conf.d/00-openssl.conf && ldconfig \
    && mkdir -p /etc/nginx /etc/certs /var/cache/nginx /var/log/nginx \
